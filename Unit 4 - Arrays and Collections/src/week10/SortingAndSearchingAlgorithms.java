...
public class SortingAndSearchingAlgorithms {
  public static void main(String[] args) {
    ...
    selectionSort(arr3);
    display(arr3);
    System.out.println("\n\n-------------------------\n" + binarySearch(arr3, 5, 0, arr.length - 1));
  }
  ...
  */
  private static int[] merge(int[] nums, int[] nums1) {
    ...
    for (int k = 0; k < arr.length; k++) {
      ...
      if (i < nums.length && j < nums1.length) {
        ...
        // checks if we have stuff left in the first array
      } else if (i < nums.length) {
        arr[k] = nums[i];
        i++;
        // we have stuff left only in the second array
      } else {
        arr[k] = nums1[j];
        j++;
      }
  ...
  private static void selectionSort(int[] nums) {
    // outer loop is responsible for the division between
    // the sorted and unsorted side
    for (int i = 0; i < nums.length - 1; i++) {
      int minIndex = i;
      // inner loop is responsible for locating the smallest element
